...
class SupplyChainVisualizer:
    ...
    def visualize_seller_clusters(self, seller_data, title="Seller Performance Clusters"):
        ...
        if 'cluster_centers' in seller_data.columns:
            # This section would plot centers if provided; implementation may vary
            pass
    ...
    def create_supply_chain_dashboard(self, monthly_demand, seller_performance, forecasts, recommendations):
        ...
        try:
            ...
            plt.close()
            print(f"Dashboard created: {self.output_dir}/supply_chain_dashboard.png")
    ...
    def _plot_demand_trends(self, ax, monthly_demand):
        ...
        try:
            if monthly_demand is None or monthly_demand.empty:
                ax.text(0.5, 0.5, 'No demand data available', horizontalalignment='center', verticalalignment='center')
                return
            
            cat_col = 'product_category_name'
            if cat_col not in monthly_demand.columns:
                ax.text(0.5, 0.5, 'Missing category column in data', horizontalalignment='center', verticalalignment='center')
                return
            ...
            if count_col is None:
                ax.text(0.5, 0.5, 'No count data available', horizontalalignment='center', verticalalignment='center')
                return
            ...
            ax.text(0.5, 0.5, 'Error plotting demand trends', horizontalalignment='center', verticalalignment='center')
    ...
    def _plot_seller_performance(self, ax, seller_data):
        ...
        try:
            if seller_data is None or seller_data.empty:
                ax.text(0.5, 0.5, 'No seller data available', horizontalalignment='center', verticalalignment='center')
                return
                
            req_columns = ['prediction']
            if not all(col in seller_data.columns for col in req_columns):
                ax.text(0.5, 0.5, 'Missing required columns in seller data', horizontalalignment='center', verticalalignment='center')
                return
            ...
            ax.text(0.5, 0.5, 'Error plotting seller performance', horizontalalignment='center', verticalalignment='center')
            
    def _plot_forecast(self, ax, forecasts):
        """Helper method to plot forecast on given axis"""
        try:
            if not forecasts:
                ax.text(0.5, 0.5, 'No forecast data available', horizontalalignment='center', verticalalignment='center')
                return
                
            ax.bar(['Current', 'Forecast'], [100, 110], color=['#1f77b4', '#ff7f0e'])
            ax.set_title('Demand Forecast', fontsize=14)
            ax.set_ylabel('Relative Demand (%)', fontsize=12)
            
        except Exception as e:
            print(f"Error plotting forecast: {e}")
            ax.text(0.5, 0.5, 'Error plotting forecast', horizontalalignment='center', verticalalignment='center')
    ...
    def _plot_reorder_recommendations(self, ax, recommendations):
        ...
        try:
            if recommendations is None or recommendations.empty:
                ax.text(0.5, 0.5, 'No recommendations data available', horizontalalignment='center', verticalalignment='center')
                return
            ...
            if cat_col is None:
                ax.text(0.5, 0.5, 'Missing category column in recommendations', horizontalalignment='center', verticalalignment='center')
                return
            ...
            if sort_col is None:
                ax.text(0.5, 0.5, 'Missing value columns in recommendations', horizontalalignment='center', verticalalignment='center')
                return
                
            top_recs = recommendations.sort_values(sort_col, ascending=False).head(5)
            ax.barh(top_recs[cat_col], top_recs[sort_col], color='#1f77b4')
            ...
            ax.text(0.5, 0.5, 'Error plotting recommendations', horizontalalignment='center', verticalalignment='center')
            
    def _plot_supply_chain_kpis(self, ax):
        """Helper method to plot supply chain KPIs on given axis"""
        try:
            metrics_path = pd.compat.os.path.join(self.output_dir, "performance_metrics.csv")
            if not pd.compat.os.path.exists(metrics_path):
                ax.text(0.5, 0.5, 'No performance metrics available', horizontalalignment='center', verticalalignment='center')
                return
                
            metrics_df = pd.read_csv(metrics_path)
            if metrics_df.empty:
                ax.text(0.5, 0.5, 'Empty performance metrics file', horizontalalignment='center', verticalalignment='center')
                return
            ...
            if kpi_names:
                ...
                ax.text(0.5, 0.5, 'No KPI data available to display', horizontalalignment='center', verticalalignment='center')
                
        except Exception as e:
            print(f"Error plotting KPIs: {e}")
            ax.text(0.5, 0.5, 'Error plotting KPIs', horizontalalignment='center', verticalalignment='center')
